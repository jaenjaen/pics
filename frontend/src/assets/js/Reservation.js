  ...
  computed: {
    ...
    checkSchedule: function() {
      ...
      var startDayTime = new Date(
        this.start_date + " " + this.start_time + ":00:00"
      ).getTime();
      ...
      if (
        ...
        for (var k = 0; k < this.schedule.exceptionDate.length; k++) {
          //포함안되면 repeat가서 뒤지기
          if (
            (new Date(this.schedule.exceptionDate[k].endDate).getTime() <=
              startDayTime) |
            (new Date(this.schedule.exceptionDate[k].startDate).getTime() >=
              endDayTime)
          ) {
            for (var j = 0; j < this.schedule.repeatDate.length; j++) {
              if (this.schedule.repeatDate[j].weekDate == week[startDay]) {
                //일치하는 요일의 시작 시간 가져오기
                console.log(
                  "시간 : " +
                    parseInt(this.start_time) +
                    week[startDay] +
                    this.schedule.repeatDate[j].weekDate
                );
                if (
                  (parseInt(this.start_time) >=
                    parseInt(this.schedule.repeatDate[j].time.split("-")[0])) &
                  (parseInt(this.start_time) <
                    parseInt(this.schedule.repeatDate[j].time.split("-")[1]))
                ) {
                  msg = "";
                  return msg;
                } else {
              ...
              if (this.schedule.repeatDate[j].weekDate == week[endDay]) {
                //일치하는 요일의 종료 시간 가져오기
                if (
